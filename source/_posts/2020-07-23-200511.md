...
然后可以使用 getBean 获取 bean 的实例。ApplicationContext 接口还有其他一些检索 bean 的方法，但是理想情况下，不应该有任何一处调用这些方法的代码。应用程序不显式地调用 getBean ()方法，会让应用程序和 Spring api 解耦。例如，Spring 与 web 框架的集成为各种 web 框架组件提供了依赖注入，比如控制器和 jsf 管理的 bean，让你通过元数据(比如@AutoWire注释)声明对特定 bean 的依赖。